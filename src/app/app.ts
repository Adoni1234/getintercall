...
//   private silenceTimer: any = null;
//   private lastPartialTime = 0;
//   private readonly SILENCE_TIMEOUT_MS = 2500; // 2.5s sin partials → forzar cierre de bloque
//   private previousTranscriptionLength = 0;
//   autoScrollEnabled = true;
...
//         if (data.isNewTurn) {
//           // ── BLOQUE FINALIZADO (is_final o force close desde backend) ──────
//           // El backend envía el texto COMPLETO del turno.
//           // Descartar el currentPartial acumulado en frontend (el backend ya lo tiene completo).
//           if (currentPartial_isFromSameBlock(this.currentPartial, detectedLang)) {
//             // Reemplazar el partial con el texto final completo del backend
//             this.currentPartial = { text: newText, lang: detectedLang };
//           } else if (this.currentPartial.text.trim()) {
//             this.bloques.push({ ...this.currentPartial });
//           }

//           // Empujar como bloque definitivo
//           this.bloques.push({ text: newText, lang: detectedLang });
//           this.currentPartial = { text: '', lang: '' };

//           console.log(`✅ BLOQUE FINAL #${this.bloques.length} [${detectedLang}]: "${newText.substring(0, 60)}..."`);

//         } else if (data.isNewBlock) {
//           // ── NUEVO BLOQUE tras reformulación del backend ───────────────────
//           // El backend detectó que AssemblyAI reinició el contexto.
//           // Ya cerró el bloque anterior. Iniciar uno nuevo en el frontend.
//           this.currentPartial = { text: newText, lang: detectedLang };
//           console.log(`🆕 NUEVO BLOQUE [${detectedLang}]: "${newText.substring(0, 60)}..."`);
...
//           if (this.currentPartial.text) {
//             this.currentPartial.text += ' ' + newText;
//             // Actualizar idioma si detectamos cambio (para los primeros tokens)
//             const wordCount = this.currentPartial.text.split(/\s+/).filter(Boolean).length;
//             if (wordCount < 5) {
//               this.currentPartial.lang = detectedLang;
//             }
...
//             this.currentPartial = { text: newText, lang: detectedLang };
//           }
//           console.log(`📝 PARTIAL [${detectedLang}]: "${this.currentPartial.text.substring(0, 60)}..."`);
//         }
...
//   // Timer de seguridad en frontend: si pasan 2.5s sin partials, cerrar bloque actual
//   private startSilenceTimer(): void {
//     this.silenceTimer = setInterval(() => {
//       if (!this.isRecording) return;
//       if (!this.currentPartial.text.trim()) return;
...
//   // ── Detección de idioma (fallback si backend no envía) ───────────────────
//   private detectLanguageFrontend(text: string): 'es' | 'en' {
//     const cleanText = text.toLowerCase().trim();
//     if (/[áéíóúñ¿¡]/i.test(cleanText)) return 'es';
//     const spanishPattern = /\b(de|del|el|la|los|las|un|una|está|están|son|es|como|qué|cómo|por|para|con|sin|pero|y|o|mi|tu|su|me|te|se|lo|le|ha|he|sido|sé|vamos|hacer|entonces|solo|más|nada|esto|no|que|muy|aquí|allí|bien|mal|todo|siempre|nunca|cuando|donde|mucho|poco|grande|nuevo|bueno|malo|si|sí|ver|ir|voy|va|hago|dice|ser|estar|tener|tengo|tiene|poder|puedo|querer|quiero|deber|debe|año|día|vez|cosa|gente|tiempo|vida|casa|ciudad|centro|desde|hasta|otro|mismo|cada|todos|estamos)\b/gi;
//     const words = cleanText.split(/\s+/).filter(w => w.length > 0);
//     const spanishMatches = cleanText.match(spanishPattern);
//     const spanishWordCount = spanishMatches ? spanishMatches.length : 0;
//     const spanishRatio = words.length > 0 ? spanishWordCount / words.length : 0;
...
//   removeTranslation(index: number): void { this.translations.splice(index, 1); }
//   clearAllTranslations(): void { this.translations = []; this.snackBar.open('🧹 Traducciones limpiadas', 'OK', { duration: 1500 }); }
//   toggleTranslationPanel(): void { this.showTranslationPanel = !this.showTranslationPanel; }
//   async translateSelection(): Promise<void> {}
//   closeTranslation(): void {}
...
// // Helper fuera de la clase para evitar errores de this en el handler del socket
// function currentPartial_isFromSameBlock(partial: { text: string; lang: string }, lang: string): boolean {
//   return partial.text.trim().length > 0 && partial.lang === lang;
// }
